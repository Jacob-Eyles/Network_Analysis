...
def search(tree, search_value):
	...
	# create a linked list and append the head node.
	ll = LinkedList()
	ll.append(tree.head)

	search = True
	while search:
		# try and pop off the first node in the ll.
		try:
			nd = ll.pop(0)
			# if an attribute error, the ll is empty (gone through whole list).
		except(AttributeError):
			# print(f'node value {search_value} not found :)')
			# so we return None as no value of the search value has been found.
			return None
		
		# if the node is found, return the name.
		if nd.value == search_value:
			# print('Found node at', nd.name)
			return nd.name
		# if the node is not found in this loop, add all its daughters to the end of the ll to be checked.
...
def insertion_sort(A):
	...
	# creates an initial index of the list A.
	n = len(A)
	inds = np.arange(n)

	# starting at index 1, cycle forward with j as starting limit.
	for j in range(1,n):
		# the key is what we compare everthing below it to.
		key = A[j]
		# do the same to the index.
		indskey = inds[j]

		# create loop to cycle through indexes below.
		i = j-1

		while i>-1:
			# if the key isnt smaller, continue
			if not(A[i] > key):
				break
			# if it is smaller, promote the larger index.
			A[i+1] = A[i]
			inds[i+1] = inds[i]
			i -= 1
		# at the end, put in key where necessary and same to the indexes.
		A[i+1] = key
		inds[i+1] = indskey
...
def dijkstra(network, source_name, destination_name):
	...
	'''
	# create an unvisited set with all unvisited nodes.
	unvisited = set([nd.name for nd in network.nodes])

	# set all nodes to have no predecessor and infinite distance.
	for i in unvisited:
		node = network.get_node(i)
		node.value = [np.inf,None]

	# set the source to have no predecessor and 0 distance.
	source = network.get_node(name = source_name)
	source.value = [0,None]
	# destination node.
	destination = network.get_node(name = destination_name)

	# loop through all the unvisited nodes.
	check = True
	while check:

		# create comparison variables for lowest distance in unvisited.
		new = None
		minn = np.inf

		# find lowest distance node in unvisited.
		for nd in unvisited:
			nd = network.get_node(nd)
			if nd.value[0] < minn:
				minn = nd.value[0]
				new = nd.name

		# gets the lowest distance node and removes it from unvisited set.
		# if a network error occurs, the next node cannot be reached. This will have
		# happened before the destination was reached as the algorithm takes the shortest
		# node which hasnt been visited yet. The output is no path and infinite (unit).
		try:
			newnode = network.get_node(new)
			unvisited.remove(new)
		except:
			return destination.value[0], None

		# calculates the distance to neighbouring arc out nodes. If this is smaller than the 
		# orginal distance, replace it and put a new predecessor.
		for arc in newnode.arcs_out:
			tonode = network.get_node(arc.to_node.name)
			if (arc.weight + newnode.value[0]) < tonode.value[0]:
				tonode.value = [arc.weight + newnode.value[0], newnode.name]

		# checks if the destination has been visited. This works as the algorithm picks the shortest 
		# unvisited node to visit next. By the time the destination has been visited all the shortest options 
		# to find it would have been found so the algorithm can be cut short.
		done = True
		if destination_name in unvisited:
			done = False
		if done:
			check = False

	# if the destination node is reachable, create a path starting at the destination name and working back.
	path = [destination_name,]
	# keeps adding shortest path backwards.
	while path[-1] != source_name:
		node = network.get_node(path[-1])
		path.append(node.value[1])
	# reverses path
